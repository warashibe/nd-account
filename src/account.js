...
const userUpdate = async ({ u, set }) => {
  ...
  logging = false
  set({ user$account: user, _user$account: u, user_init$account: true })
}
...
export const login = async ({
  ...
  if (provider === "uport") {
    await login_with_uport({ set, state$, conf })
    return
  } else if (provider === "metamask") {
    await login_with_metamask({ set, state$, conf })
    return
  } else if (provider === "authereum") {
    await login_with_authereum({ set, state$, conf })
    return
  } else if (provider === "alis") {
    await login_with_alis({ set, state$, conf })
    return
  } else if (provider === "steem") {
    await login_with_steem({ set, state$, conf })
    return
  }
...
const login_with_uport = async ({ conf, set, state$, link = false }) => {
  /*
...
const login_with_authereum = async ({ conf, set, state$ }) => {
  if (R.xNil(window)) {
    set(true, "processing$util")
...
const login_with_metamask = async ({ conf, set, state$ }) => {
  if (R.xNil(window)) {
    set(true, "processing$util")
...
const login_with_alis = async ({ set, conf }) => {
...
const login_with_steem = async ({ set, state$, link = false, conf }) => {
...
export const linkAccount = async ({
  ...
  if (provider === "uport") {
    set(true, "processing$util")
    await login_with_uport({ set, state$, link: true, conf })
  } else if (provider === "steem") {
    await login_with_steem({ set, state$, link: true, conf })
    return
  } else if (provider === "alis") {
    set(true, "processing$util")
    await login_with_alis({ set, conf })
    return
  } else {
    set(true, "processing$util")
    const _provider = getProvider(provider)
